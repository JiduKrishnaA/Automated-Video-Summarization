...
# python main.py -d my_data/tvsum5_orb.h5 -s dataset_split/tvsum_splits.json --input-dim 32 --lr 1e-06 --weight-decay 0.001 --max-epoch 20 -m tvsum --save-dir tvsum5
# nohup python -u main.py -d ../my_data/summe2_googlenet.h5 -s dataset_split/summe_splits.json -m summe --gpu 2 --save-dir summe2 --verbose >> ../log/train2.o &
...
def main():
    ...
    if args.evaluate:
        ...
        for split_idx, split in enumerate(splits):
            ...
            evaluate(split_idx,model, dataset, test_keys, use_gpu)
        return
    else:
        start_epoch = 0
    ...
    for split_idx, split in enumerate(splits):
        ...
        for epoch in range(start_epoch, args.max_epoch):
            ...
            for idx in idxs:
                ...
                # scaler = preprocessing.StandardScaler().fit(seq)
        ...
        write_json(reward_writers, osp.join('models/'+args.save_dir, 'rewards%d.json'%split_idx))
        evaluate(split_idx,model, dataset, test_keys, use_gpu)
        ...
        model_save_path = osp.join('models/'+args.save_dir, 'model_' + str(split_idx) + '.pth.tar')
        save_checkpoint(model_state_dict, model_save_path)
        print("Model saved to {}".format(model_save_path))

    dataset.close()

def evaluate(split_idx, model, dataset, test_keys, use_gpu):
    print("==> Test")
    # d3 = h5py.File('../my_data/summe30_googlenet.h5', 'r')
    d3 = h5py.File('../my_data/tvsum2_googlenet.h5', 'r')
    with torch.no_grad():
        model.eval()
        fms = []
        ...
        folder = 'models/' + args.save_dir
        if not os.path.exists(folder):
            os.makedirs(folder)
        h5_res = h5py.File(osp.join(folder, 'result%d.h5' % split_idx), 'w')
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            # Neglect user summary entirely
            user_summary = np.zeros(num_frames, dtype=np.float32)

            machine_summary = vsum_tools.generate_summary(probs, cps, num_frames, nfps, positions)
            ed = time.time()
            print(key, ed - st)
            fm, _, _ = vsum_tools.evaluate_summary(machine_summary, eval_metric)
            fms.append(fm)
            ...
            h5_res.create_dataset(key + '/time3', data=ed - st)
    d3.close()
    #if args.verbose:
    ...
    # del h5_res['mean_fm']
    h5_res.create_dataset('mean_fm', data=mean_fm)
