...
dataFile = "F:\mainproject\ydata-tvsum50-v1_1\ydata-tvsum50-matlab\matlab\ydata-tvsum50.mat"
...
def get_tvsum():
    ...
    for i in range(50):
        ...
        name_ref = mat['tvsum50/video'][i, 0]
        # Correct way to obtain the reference data
        name_data = mat[name_ref]
        if isinstance(name_data, h5py.Reference):
            # Convert the reference to a string
            video_name = mat[name_data].astype('str')[0] + ".mp4"
        else:
            video_name = "".join(chr(int(i)) for i in name_data) + ".mp4"

        if video_name in d:
            print(f"Keys in {video_name}: {list(d[video_name].keys())}")
            if video_name+'/picks' in d:
                ...
                if d[video_name+'/picks'][()].shape[0] > gt_score.shape[0]:
                    gt_score = np.pad(gt_score, (0, d[video_name+'/picks'][()].shape[0] - gt_score.shape[0]), 'constant')
            else:
                # Handle the case where the key is not present in the file
                print(f"Warning: {video_name+'/picks'} not found in {video_name}.")
        else:
            print(f"Warning: {video_name} not found in the HDF5 file.")
